...
In vielen Softwareprojekten gibt es keine Architekturdokumentationen. Auffällig ist, das viele OpneSource-Projekte die Architektur nicht dokumentieren.(\cite{Role}S.246)\\
...
\paragraph{Folgen}
Die Software wird zu komplex und zu teuer. Außerdem ist es möglich, dass sollten Änderungen in anderen Dimensionen von Nöten sein diese sogar schwieriger werden als wenn die Software mit einer einfacheren Architektur implementiert worden wäre.
...
Es ist schlichtweg unmöglich eine Softwarearchitektur mit vertretbaren Kosten und Risiken zu entwickeln, welche alle möglichen Änderungen unterstützt.(\cite{SSA:12}S.561)
\paragraph{Ursachen}
Bei der Entscheidung, welche Arten von Änderungen unterstützt werden, wurde falsch entschieden.
\paragraph{Folgen}
Die Unterstützung von Änderungen verurscht Overhead im Design, in der Implementierung und häufig auch zur Laufzeit. Daher verursacht es unnötige Kosten, wenn die Änderungen die Unterstützt werden nie kommen.
\paragraph{Risikoreduktion}
Die Bereitstellung der Unterstützung bestimmter Arten von Änderungen durch die Architektur sollte nur mit ausreichender Sicherheit, dass diese auch eintreten.
\paragraph{Folgenreduktion}
...
Es kann sein, dass die verwendeten Komponenten nicht mehr verfügbar sind. Bei Hardware könnte es sein dass diese Kaputt geht und evtl. nicht mehr hergestellt wird und bei Software z.B. dass Verträge auslaufen.\\
...
\subparagraph{3.}
Die Auswirkungen durch Änderungen der spezifischen Komponenten sollte durch eine entsprechende Abstraktion der der Schnittstellen zu diesen erfolgen.
(\cite{SSA:12}S.562)
%%________________________________________________________________
...
Um eine Entsprechende Enticklungsumgebung wiederherzustellen müssen folgende Fragen beantwortet werden können.\\
...
Wird eine Bestimmte OS version benötigt? oder Bestimmtes Model von Hardwarekomponenten? Oder sind diese durch neuere Versionen ersetzbar?\\
...
Das Deployen in eine Testumgebung birgt normalerweise kein Risiko. Sollten dabei Fehler auftreten, so können diese in Ruhe behoben und dann nochmals deployt werden, ohne das Benutzer betroffen sind.\\
